...
│   ├── api-common.md          # 通用接口文档（统一接口前缀、状态码、返回格式说明）
│   ├── enum-common.md         # 通用枚举文档（前后端共用枚举定义、值说明）
│   └──协作规范.md              # 团队协作规范（分支管理、提交规范、联调流程）
├── skills/                    # 通用技能文档目录（提升开发效率，前后端共用）
│   ├── debug技巧.md            # 通用调试技巧（前后端联调排错、Claude使用调试）
│   ├── 常用命令.md              # 常用命令汇总（Git、前端、后端、脚本相关命令）
│   └── 依赖版本说明.md          # 依赖版本规范（前端npm、后端Maven依赖版本及兼容说明）
└── rules/                     # 全局通用规则目录
...
├── docs/                      # 前端独有文档目录
│   ├── 前端技术文档.md          # 前端技术栈说明（Vue3、TS、Pinia等使用规范）
│   ├── 组件开发规范.md          # 前端组件开发标准（复用、命名、样式规范）
│   └── 前端部署文档.md          # 前端部署流程（本地运行、打包、上线步骤）
├── skills/                    # 前端技能文档目录
│   ├── 前端性能优化技巧.md      # 前端性能优化（首屏加载、重渲染、缓存优化）
│   ├── Vue3常用技巧.md         # Vue3核心用法、Composition API实战技巧
│   └── 前端问题排查手册.md      # 常见前端问题（报错、兼容、联调）排查方法
├── .gitignore                # 前端Git忽略文件（忽略Claude配置、符号链接、依赖等）
├── package.json              # 前端依赖配置文件（标注核心依赖及版本）
